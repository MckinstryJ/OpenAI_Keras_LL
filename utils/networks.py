...
def get_session():
    ...
    return tf.Session(graph=tf.compat.v1.get_default_graph(), config=config)


def tfSummary(tag, val):
    """ Scalar Value Tensorflow Summary
    """
    return tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=tag, simple_value=val)])
